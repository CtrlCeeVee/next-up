...
2. Design pass (T-007): done and verified 2026-09-18 (26ec17c, plus the colour-consolidation review round aed7df7). FAQ (T-008) kept open by the owner; GSC showed no question-style queries between 2026-04-28 and 2026-09-16.
3. Switchover done 2026-09-18: Vercel root directory `code/web`, Next.js preset, `RESEND_API_KEY` in Production, apex redirect 308, PR #23 merged (d17b0ea), sitemap with 9 URLs resubmitted through the GSC API. Verified live the same day: per-page titles, canonicals and JSON-LD in the raw HTML; unknown URLs 404; `/auth`, `/leagues`, `/league/2`, `/league/3`, `/leaderboard`, `/profile/*` and `/reset-password` 308 to their targets; Googlebot and ClaudeBot user agents get 200. Still to do by hand: "Request indexing" on the five key URLs. Post-deploy Lighthouse mobile (simulated throttling, run from Johannesburg): club page performance 89, LCP 2.6 s; homepage 79 and 65 across two runs, LCP 4.3 to 5.4 s, FCP 1.5 to 3.0 s; CLS 0, accessibility 98, best practices and SEO 100 on both. The homepage LCP element is the hero phone image (31 KB WebP, `fetchpriority=high`); total transfer 558 KB of which 325 KB is script. The homepage misses the performance target; confirm with PageSpeed Insights before optimising, because the two local runs disagree by 14 points.
